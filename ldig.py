...
class ldig(object):
    ...
    def load_labels(self):
        with open(self.labels, 'rb') as f:
            ...
            return json.load(io.StringIO(str_response))
...
def likelihood(param, labels, trie, filelist, options):
    ...
    label, text, org_text = normalize_text(filelist)
    ...
    #print(predict_lang)
    #print(org_text)
    #print('{0}\{1}\{2}'.format(label, predict_lang, org_text))
    ss = bytes(predict_lang, 'utf-8')

    #print(predict_lang.decode('utf-8'))
    #pp = ss.decode("utf-8")
    #ppp = str(ss, "utf-8")
    #print(ppp)
    #sys.stdout.buffer.write(bytes(label, 'utf-8')+ bytes(" ", 'utf-8') + ss + bytes(" ", 'utf-8') + bytes(org_text,
    # 'utf-8') +  bytes("\n", 'utf-8'))
    #f.close()
    ...
    #sss = str(log_likely, 'utf-8')
  #  print("{0}".format(type(log_likely)))
    return predict_lang
